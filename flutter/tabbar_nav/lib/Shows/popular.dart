...
class PopularState extends State<Popular> {
  ...
  Widget build(BuildContext context) {
    return PopularShowsGridView(
      popResults: widget.data['results'],
    );
  }
}

class PopularShowsGridView extends StatefulWidget {
  final List popResults;

  PopularShowsGridView({
    Key key,
    this.popResults,
  });
  ...
  @override
  PopularShowsGridViewState createState() => new PopularShowsGridViewState();
}

class PopularShowsGridViewState extends State<PopularShowsGridView> {
  ...
  _getMoreData() async {
    if (!widget.isPerformingRequest) {
      ...
      http.Response res = await http.get(
          Uri.encodeFull(
              "https://api.themoviedb.org/3/tv/popular?api_key=12a3d32040053bd41f3567e684bcd83a&language=en-US&page=${page}"),
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        if (index == widget.popResults.length) {
          ...
          return PopularList(
            popularShow: widget.popResults[index],
          );
        }
      },
...
class PopularList extends StatelessWidget {
  final Map<String, dynamic> popularShow;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        onTap: () => _onTapGridItem(
            popularShow['id'], popularShow['poster_path'], context),
        child: Column(
          ...
          children: <Widget>[
            Expanded(
              child: SizedBox(
                width: 120.0,
                child: Hero(
                  tag: 'poster ${popularShow['id']}',
                  child: Material(
                    color: Colors.transparent,
                    child: InkWell(
                      child: CachedNetworkImage(
                        imageUrl:popularShow['poster_path'] != null
                            ? "https://image.tmdb.org/t/p/w342/${popularShow['poster_path']}"
